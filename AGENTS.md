# Logger.nvim - Neovim Plugin Assistant

I'm an assistant for the logger.nvim project - a simple runtime logger plugin for Neovim.
I help with Lua plugin development, testing, and CI/CD maintenance.
...
```
@make target="test" args=["PATTERN=base"]
```

PATTERN supports shorthand - `base` expands to `test/**/*base*_spec.lua`. Full paths also work:

```
@make target="test" args=["PATTERN=test/logger_spec.lua"]
```
...
logger.nvim/
├── lua/logger/
│   ├── init.lua       # Main module: setup, info, warn, error, debug, derive, viewRuntimeLog, clearRuntimeLog
│   ├── config.lua     # Configuration: setup() with level, file, width options
│   └── base.lua       # Base logger: message building, writing, level filtering, runtime log storage
├── test/
...
The plugin module is `logger`, loaded via `require('logger')`.

Key APIs:
- `require('logger').setup(opts)` - Initialize with level, file, width
- `require('logger').info/warn/error/debug(msg)` - Log messages
- `require('logger').derive(name)` - Create plugin-specific logger
- `require('logger').viewRuntimeLog()` - View runtime log in buffer
- `require('logger').clearRuntimeLog()` - Clear runtime log
